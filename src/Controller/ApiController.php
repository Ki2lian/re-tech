...
class ApiController extends AbstractController
{
     ...
     */
    public function singleAnnonce(AnnonceRepository $annonce, int $id, string $token): Response
    {
        if ($token === $_ENV['API_TOKEN']) {
     ...
     */
    public function singleTag(TagRepository $tag, int $id, string $token, int $skip = 0, int $fetch = 10): Response
    {
        if ($skip < 0 || $fetch <= 0) return $this->json(["code" => 400, "message" => "Bad request"], 400);
        if ($token === $_ENV['API_TOKEN']) {
            $data = $tag->find($id);
     ...
     */
    public function singleTicket(TicketRepository $ticket, int $id, string $token): Response
    {
        if ($token === $_ENV['API_TOKEN']) {
            $data = $ticket->find($id);
